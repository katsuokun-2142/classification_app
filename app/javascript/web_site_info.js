...
function web_site_info (){
  //リクエストを送信する処理
  // console.log("イベント発火");
  const deleteButtons = document.querySelectorAll('.site-delete-button');
      ...
      if (result) {
        ...
        .then(response => {
          if (response.ok) {
            // 削除が成功した場合、要素をDOMから削除
            webSiteInfoElement.remove();
            const remainingItems = document.querySelectorAll('.site-delete-button'); // .delete-buttonは各アイテムにつけられたクラス
            if (remainingItems.length === 0) {
              // アイテムが0になったら、前のページに移動
              const currentPageNumber = parseInt(new URLSearchParams(window.location.search).get('page'), 10);
              if (currentPageNumber && currentPageNumber > 1) {
